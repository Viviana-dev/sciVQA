...
def process_single_data_row(row: pd.Series, split_name: str):
    image_file = load_real_image_path(row["image_file"], **{split_name: True})
    ...
    # Resize & pad
    img_sq, pad_x, pad_y = resize_and_pad(img_full.copy(), max_pixels=2000000)
    canvas_w, canvas_h = img_sq.size
    scale_x = (canvas_w - 2 * pad_x) / orig_w
    scale_y = (canvas_h - 2 * pad_y) / orig_h

    boxes = extract_ocr_boxes(img_sq, lang="en", conf_thr=60)
